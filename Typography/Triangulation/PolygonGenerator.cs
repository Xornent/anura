...
namespace Anura.Typography.Triangulation {
    public class PolygonGenerator {
        ...
        public static Polygon RandomCircleSweep (double scale, int vertexCount) {
            ...
            TriangulationPoint[] array = new TriangulationPoint[vertexCount];
            for (int i = 0; i < vertexCount; i++) {
                ...
                TriangulationPoint triangulationPoint = array[i] = new TriangulationPoint (num * Math.Cos (PI_2 * (double) i / (double) vertexCount), num * Math.Sin (PI_2 * (double) i / (double) vertexCount));
            }
