...
namespace Anura.JavaScript.Runtime.Interop {
    ...
    public sealed class ObjectWrapper : ObjectInstance, IObjectWrapper {
        ...
        public override PropertyDescriptor GetOwnProperty (string propertyName) {
            ...
            var explicitProperties = (from iface in interfaces from iprop in iface.GetProperties () where EqualsIgnoreCasing (iprop.Name, propertyName) select iprop).ToArray ();
            ...
            var explicitMethods = (from iface in interfaces from imethod in iface.GetMethods () where EqualsIgnoreCasing (imethod.Name, propertyName) select imethod).ToArray ();
            ...
            var explicitIndexers =
                (from iface in interfaces from iprop in iface.GetProperties () where iprop.GetIndexParameters ().Length != 0 select iprop).ToArray ();
