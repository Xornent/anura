            ...
            if (_memoryLimit > 0) {
                if (GetAllocatedBytesForCurrentThread != null) {
                    ...
                    if (memoryUsage > _memoryLimit) {
                        Anura.JavaScript.Runtime.ExceptionHelper.ThrowMemoryLimitExceededException($"Script has allocated {memoryUsage} but is limited to {_memoryLimit}");
                    }
                } else {
                    Anura.JavaScript.Runtime.ExceptionHelper.ThrowPlatformNotSupportedException("The current platform doesn't support MemoryLimit.");
                }
            }
        }
