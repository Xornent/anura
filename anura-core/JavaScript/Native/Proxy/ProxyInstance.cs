...
namespace Anura.JavaScript.Native.Proxy
{
    public class ProxyInstance : FunctionInstance, IConstructor
    {
        ...
        public ObjectInstance Construct(JsValue[] arguments, JsValue newTarget)
        {
            ...
            if (!TryCallHandler(TrapConstruct, new[] { _target, argArray, newTarget }, out var result))
            {
                ...
                {
                    return Anura.JavaScript.Runtime.ExceptionHelper.ThrowTypeError<ObjectInstance>(_engine);
                }
        ...
        public override JsValue Get(JsValue property, JsValue receiver)
        {
            ...
            if (targetDesc != PropertyDescriptor.Undefined)
            {
                ...
                {
                   Anura.JavaScript.Runtime.ExceptionHelper.ThrowTypeError(_engine, $"'get' on proxy: property '{property}' is a non-configurable accessor property on the proxy target and does not have a getter function, but the trap did not return 'undefined' (got '{result}')");
                }
            }
        ...
        public override bool Delete(JsValue property)
        {
            ...
            if (success)
            {
                ...
                {
                    Anura.JavaScript.Runtime.ExceptionHelper.ThrowTypeError(_engine, $"'deleteProperty' on proxy: trap returned truish for property '{property}' which is non-configurable in the proxy target");
                }
            }
        ...
        protected override ObjectInstance GetPrototypeOf()
        {
            ...
            {
                Anura.JavaScript.Runtime.ExceptionHelper.ThrowTypeError(_engine, "'getPrototypeOf' on proxy: trap returned neither object nor null");
            }
        ...
        private bool TryCallHandler(JsValue propertyName, JsValue[] arguments, out JsValue result)
        {
            ...
            if (!handlerFunction.IsNullOrUndefined())
            {
                ...
                {
                    return Anura.JavaScript.Runtime.ExceptionHelper.ThrowTypeError<bool>(_engine, $"{_handler} returned for property '{propertyName}' of object '{_target}' is not a function");
                }
        ...
        internal void AssertNotRevoked(JsValue key)
        {
            if (_handler is null)
            {
                Anura.JavaScript.Runtime.ExceptionHelper.ThrowTypeError(_engine, $"Cannot perform '{key}' on a proxy that has been revoked");
            }
        }
