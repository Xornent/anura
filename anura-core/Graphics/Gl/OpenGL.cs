...
namespace Anura.Graphics.Gl {
    ...
    /// <summary>
    /// 对不同系统上的 OpenGL 支持的共有函数的抽象类，通过调用此类的不同子类（这些
    /// 子类是系统特异的）来访问 OpenGL 的标准
    /// 
    /// 在 [Anura.Graphics.Gl.Current] 静态类中，我们在系统初始化时就根据系统自动
    /// 选择了相适配的 OpenGL 实现。所以，你可以直接调用 Current 类
    /// </summary>
    internal abstract class OpenGL {
        ...
        /// 指定测试函数
        /// </summary>
        /// <param name="func">指定比较函数。符号常量: never, less, equal, lequal, greater, notequal,
        /// gequal 和 always 都被接受。初始值是 always</param>
        /// <param name="ref_notkeword">指定与传入的alpha值进行比较的引用值。该值被限制在 0 到 1 之间，其中
        /// 0 表示可能的最低 alpha 值，1 表示可能的最高值。初始参考值为 0</param>
        public abstract void AlphaFunc (uint func, float ref_notkeword);

        /// <summary>
        ///确定纹理是否已加载到纹理内存中。
        ///</summary>
        ///<param name ="n">指定要查询的纹理数量。</param>
        ///<param name ="textures">指定一个数组，其中包含要查询的纹理的名称。</param>
        ///<param name ="residences">指定一个数组，在该数组中返回纹理驻留状态。在相应的 Residences
        /// 元素中返回以 texture 元素命名的纹理的驻留状态。</param>
        public abstract byte AreTexturesResident (int n, uint[] textures, byte[] residences);

        /// <summary>
        /// 使用指定的顶点数组元素渲染顶点
        /// </summary>
        /// <param name="i">指定已启用的顶点数据数组的索引</param>
        public abstract void ArrayElement (int i);

        public abstract void Begin (uint mode);

        /// <summary>
        /// 创建纹理后调用此函数以最终完成创建，或使现有纹理成为当前纹理
        /// </summary>
        /// <param name="target">纹理类型, e.g TEXTURE_2D.</param>
        /// <param name="texture">GL纹理对象.</param>
        public abstract void BindTexture (uint target, uint texture);

        /// <summary>
        /// 绘制位图
        /// </summary>
        /// <param name="width">指定位图图像的像素宽度.</param>
        /// <param name="height">指定位图图像的像素高度.</param>
        /// <param name="xorig">指定位图图像中原点的位置。原点是从位图的左下角测量的，右和上是正轴.</param>
        /// <param name="yorig">指定位图图像中原点的位置。原点是从位图的左下角测量的，右和上是正轴.</param>
        /// <param name="xmove">指定绘制位图后要添加到当前栅格位置的x和y偏移量.</param>
        /// <param name="ymove">指定绘制位图后要添加到当前栅格位置的x和y偏移量.</param>
        /// <param name="bitmap">指定位图图像的地址.</param>
        ...
        public abstract void MultiDrawArrays (uint mode, int[] first, int[] count, int drawcount);
